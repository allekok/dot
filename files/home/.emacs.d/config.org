...
* allekok
  #+begin_src emacs-lisp
    ...
    (global-set-key (kbd "C-x j")
		    (lambda () (interactive)
		      (chromium "--app=http://localhost/my-functions/site/")))
  ...
  #+begin_src emacs-lisp
	 ...
	 (lambda (b)
	   (when (or (minibufferp b) (string-match-p " *Echo Area" (buffer-name b)))
	     (with-current-buffer b
	       (when (and (> (string-width (buffer-string)) 0)
			  (string= (substring (buffer-string) 0 1) " "))
		 (setq pass t)
		 (delete-region (point-min) (point-max))))))
	 (buffer-list))
	(when (eq pass t)
	  (cancel-timer mode-line-hide-timer)
	  (setq mode-line-hide-timer nil))))
    ...
    (defun mode-line-show ()
	 ...
	 (when (or (minibufferp b) (string-match-p " *Echo Area" (buffer-name b)))
       ...
       (buffer-list))
      (when (eq mode-line-hide-timer nil)
	(setq mode-line-hide-timer
	      (run-with-timer 10 1 'mode-line-hide))))
