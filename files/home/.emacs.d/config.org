...
* font
  #+begin_src emacs-lisp
    (when (eq 'x (window-system))
      (let ((ara (font-spec :family "Vazir-Code-WOL"))
...
* amusement
  #+begin_src emacs-lisp
    ...
    ;; Amusements
    (defun random-wiki ()
      (chromium (format "--app=https://%s.wikipedia.org/wiki/Special:Random"
			(one-of '("ckb" "en" "fa")))))
    ...
    (defun random-poem ()
      (chromium "--app=https://allekok.ir/script/php/random-poem.php?redirect"))
...
* appearance
  #+begin_src emacs-lisp
    ...
    (setq mode-line-timer (run-with-timer 1 7 'mode-line-show))
